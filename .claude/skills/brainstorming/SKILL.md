...
7. **Finalize Phase**: Use `AskUserQuestion` tool to ask what the user wants to do next:
   - **Create GitHub Issue** → Run `/issue` slash command, passing the brainstorm report path as argument. This creates a pipeline-ready issue with labels (`pipeline` + `ready_for_dev`) so `looper.sh` can pick it up.
   - **Create Implementation Plan** → Run `/plan:fast` or `/plan:hard` slash command based on complexity. Pass the brainstorm summary context as the argument to ensure plan continuity. **CRITICAL:** The invoked plan command will create `plan.md` with YAML frontmatter including `status: pending`.
   - **Both** → First run `/issue`, then `/plan:fast` or `/plan:hard`.
   - **Neither** → End the session.
